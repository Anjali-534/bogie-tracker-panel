...
// LiveMapPanel is the shared map+stats+search widget behind both the
// dedicated /tracker/live-map page and its compact embed on the /tracker
// dashboard (between the stat-card row and Recent Shipments) — one source of
// truth for the polling/marker/search logic so the two surfaces can never
// drift out of sync. showHeader/mapHeightClass are the only two things that
// differ between the two call sites; everything else (data, cards, search,
// expand-to-fullscreen) is identical.
export default function LiveMapPanel({
  showHeader = true,
  mapHeightClass = 'w-full h-[60vh] lg:h-[calc(100vh-320px)] min-h-[350px] lg:min-h-[500px] rounded-2xl overflow-hidden border border-gray-100',
}: {
  showHeader?: boolean;
        ...
        >
          ...
          <div className={mapExpanded ? 'relative flex-1 min-h-0' : 'relative'}>
            <OlaMap
              ...
              className={mapExpanded ? 'w-full h-full' : mapHeightClass}
            />
            {!mapExpanded && (
              <button
